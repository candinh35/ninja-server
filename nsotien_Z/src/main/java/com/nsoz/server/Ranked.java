...
public class Ranked {
    ...
    public static void initTopCaoThu() {
        try {
            ...
            PreparedStatement stmt = conn.prepareStatement(
                    "SELECT `name`, \n" +
                            "       CAST(JSON_EXTRACT(data, '$.exp') AS UNSIGNED) AS `exp`, \n" +
                            "       CAST(JSON_EXTRACT(data, '$.levelUpTime') AS UNSIGNED) AS `levelUpTime`,\n" +
                            "       CASE WHEN JSON_EXTRACT(data, '$.duatop') = 'true' THEN 1 ELSE 0 END AS `duatop`\n" +
                            "FROM players \n" +
                            "WHERE `server_id` = ? \n" +
